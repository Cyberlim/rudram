...
export default function ordersPage() {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string, uid: string, orderNumber: string) => {
    try {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 ease-out pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-gray-100">Orders Management</h1>
          <p className="text-gray-500 mt-1">Manage and view all your orders in one place.</p>
        </div>
              ...
              {loading ? (
                ...
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:bg-[#0B0B1A]/50 transition-colors group">
                    ...
                    <td className="py-4 px-6 font-medium text-gray-900 dark:text-gray-100">#ORD-{item.orderNumber}</td>
                    <td className="py-4 px-6 text-gray-600">{item.deliveryDetails?.name || 'Guest'}</td>
                    <td className="py-4 px-6 text-gray-600">{item.date}</td>
                    <td className="py-4 px-6 font-medium text-gray-900 dark:text-gray-100">₹{item.totalAmount?.toLocaleString()}</td>
                    <td className="py-4 px-6">
                      <div className="relative inline-block w-40">
                        <select
                          value={item.status}
                          onChange={(e) => handleStatusChange(item.id, e.target.value, item.uid, item.orderNumber)}
